"""Persistent operations for the shared ImmoTeam/alpha (immomakler) filters,
listings, deliveries, and status.

Mirrors `propotsdam_store.py` closely, minus everything district-related — the
feed has no reliable Stadtteil vocabulary to filter on, so a filter here is
just rooms/area/Kaltmiete bounds. Listings carry a `source` field (which of
the two sites first supplied this Objekt-ID) purely for diagnostics — the
listing_key itself is already deduplicated across both sites upstream in
`regiomakler_monitor.py`.
"""
...
from user_jobs import regiomakler_matching, regiomakler_parser
...
def parse_optional_number(text) -> Optional[float]:
    return regiomakler_parser.parse_decimal(text)


def filter_to_dict(row: RegiomaklerFilter) -> Dict:
    return {
...
def listing_to_dict(row: RegiomaklerListing) -> Dict:
    return {
...
def create_filter(
    ...
    try:
        ...
        now = utc_now()
        for listing_row in session.query(RegiomaklerListing).filter(
            RegiomaklerListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
            if regiomakler_matching.matches_filter(listing, filter_data):
                session.add(RegiomaklerDelivery(
                    filter_id=filter_id,
...
def update_filter(
    ...
    session = DBSession()
    try:
        row = session.query(RegiomaklerFilter).filter(
            RegiomaklerFilter.filter_id == int(filter_id),
            RegiomaklerFilter.user_id == int(user_id),
        ).first()
        if not row:
            return False
        ...
        filter_data = filter_to_dict(row)
        now = utc_now()
        session.query(RegiomaklerDelivery).filter(RegiomaklerDelivery.filter_id == int(filter_id)).delete()
        for listing_row in session.query(RegiomaklerListing).filter(
            RegiomaklerListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
...
def list_filters(user_id: Optional[int] = None, active_only: bool = False) -> List[Dict]:
    session = DBSession()
    try:
        query = session.query(RegiomaklerFilter)
        if user_id is not None:
            query = query.filter(RegiomaklerFilter.user_id == int(user_id))
        if active_only:
            query = query.filter(RegiomaklerFilter.active.is_(True))
        return [filter_to_dict(row) for row in query.order_by(RegiomaklerFilter.filter_id.asc()).all()]
    finally:
        session.close()
...
def set_filter_active(filter_id: int, active: bool, user_id: Optional[int] = None) -> bool:
    session = DBSession()
    try:
        query = session.query(RegiomaklerFilter).filter(RegiomaklerFilter.filter_id == int(filter_id))
        if user_id is not None:
            query = query.filter(RegiomaklerFilter.user_id == int(user_id))
        row = query.first()
        if not row:
...
def delete_filter(filter_id: int, user_id: Optional[int] = None) -> bool:
    ...
    try:
        ...
        if not row:
            return False
        session.query(RegiomaklerDelivery).filter(
            RegiomaklerDelivery.filter_id == int(filter_id)
    ...
    try:
        ...
        for listing in listings:
            ...
            if not key:
                continue
            seen.add(key)
            row = session.query(RegiomaklerListing).filter(RegiomaklerListing.listing_key == key).first()
            if row is None:
                row = RegiomaklerListing(listing_key=key, first_seen_at=now)
                session.add(row)
            ...
            count += 1
        if seen:
            session.query(RegiomaklerListing).filter(~RegiomaklerListing.listing_key.in_(seen)).update(
                {"is_active": False}, synchronize_session=False
            )
        else:
            session.query(RegiomaklerListing).filter(RegiomaklerListing.is_active.is_(True)).update(
                {"is_active": False}, synchronize_session=False
            )
        session.commit()
        return count
...
def list_active_listings() -> List[Dict]:
    session = DBSession()
    try:
        rows = session.query(RegiomaklerListing).filter(RegiomaklerListing.is_active.is_(True)).all()
...
def list_active_listings_since(cutoff: datetime) -> List[Dict]:
    ...
    session = DBSession()
    try:
        rows = session.query(RegiomaklerListing).filter(
            RegiomaklerListing.is_active.is_(True), RegiomaklerListing.first_seen_at >= cutoff
        ).all()
        return [listing_to_dict(row) for row in rows]
    finally:
        session.close()
...
def delivered_pairs() -> Set[Tuple[int, str]]:
    session = DBSession()
    try:
        return {(int(row.filter_id), str(row.listing_key)) for row in session.query(RegiomaklerDelivery).all()}
    finally:
        session.close()
...
def mark_delivered(filter_id: int, listing_key: str) -> None:
    session = DBSession()
    try:
        exists = session.query(RegiomaklerDelivery).filter(
            RegiomaklerDelivery.filter_id == int(filter_id),
            RegiomaklerDelivery.listing_key == str(listing_key),
        ).first()
        if not exists:
            session.add(RegiomaklerDelivery(filter_id=int(filter_id), listing_key=str(listing_key), sent_at=utc_now()))
            session.commit()
    finally:
        session.close()
    ...
    for filt in filters:
        ...
        for listing in listings:
            ...
            if not key or (filter_id, key) in delivered:
                continue
            if regiomakler_matching.matches_filter(listing, filt):
...
def record_status(status: str, listings_count: int = 0, error: str = "") -> None:
    session = DBSession()
    try:
        row = session.query(RegiomaklerStatus).filter(RegiomaklerStatus.key == STATUS_KEY).first()
        if row is None:
            row = RegiomaklerStatus(key=STATUS_KEY)
            session.add(row)
