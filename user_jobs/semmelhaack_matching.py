"""Matching helpers for SEMMELHAACK listings and user filters.

Unlike Immowelt/ProPotsdam, SEMMELHAACK's page carries no Stadtteil/district
information at all — only rooms, area, and Kaltmiete. A filter here is just
those three bounds; there is nothing to match against a district.
"""
...
def _num(value: Any) -> Optional[float]:
    ...
    try:
        ...
        return semmelhaack_parser.parse_decimal(value)
...
def format_notification(listing: Dict[str, Any], lang: str = "uk") -> str:
    return semmelhaack_parser.format_listing_message(listing, lang=lang)
