"""Persistent operations for locals® filters, listings, deliveries, and status.
...
from user_jobs import locals_matching, locals_parser
...
def parse_optional_number(text) -> Optional[float]:
    return locals_parser.parse_decimal(text)


def filter_to_dict(row: LocalsFilter) -> Dict:
    return {
...
def listing_to_dict(row: LocalsListing) -> Dict:
    return {
...
def create_filter(
    ...
    try:
        ...
        now = utc_now()
        for listing_row in session.query(LocalsListing).filter(
            LocalsListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
            if locals_matching.matches_filter(listing, filter_data):
                session.add(LocalsDelivery(
                    filter_id=filter_id,
...
def update_filter(
    ...
    session = DBSession()
    try:
        row = session.query(LocalsFilter).filter(
            LocalsFilter.filter_id == int(filter_id),
            LocalsFilter.user_id == int(user_id),
        ).first()
        if not row:
            return False
        ...
        filter_data = filter_to_dict(row)
        now = utc_now()
        session.query(LocalsDelivery).filter(LocalsDelivery.filter_id == int(filter_id)).delete()
        for listing_row in session.query(LocalsListing).filter(
            LocalsListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
...
def list_filters(user_id: Optional[int] = None, active_only: bool = False) -> List[Dict]:
    session = DBSession()
    try:
        query = session.query(LocalsFilter)
        if user_id is not None:
            query = query.filter(LocalsFilter.user_id == int(user_id))
        if active_only:
            query = query.filter(LocalsFilter.active.is_(True))
        return [filter_to_dict(row) for row in query.order_by(LocalsFilter.filter_id.asc()).all()]
    finally:
        session.close()
...
def set_filter_active(filter_id: int, active: bool, user_id: Optional[int] = None) -> bool:
    session = DBSession()
    try:
        query = session.query(LocalsFilter).filter(LocalsFilter.filter_id == int(filter_id))
        if user_id is not None:
            query = query.filter(LocalsFilter.user_id == int(user_id))
        row = query.first()
        if not row:
...
def delete_filter(filter_id: int, user_id: Optional[int] = None) -> bool:
    ...
    try:
        ...
        if not row:
            return False
        session.query(LocalsDelivery).filter(
            LocalsDelivery.filter_id == int(filter_id)
...
def known_listing_keys() -> Set[str]:
    """Ключі оголошень, які вже є в базі — щоб не ходити за їхньою повною
    ціною вдруге: вона не змінюється щопівгодини."""
    session = DBSession()
    try:
        return {str(row.listing_key) for row in session.query(LocalsListing.listing_key).all()}
    finally:
        session.close()
    ...
    try:
        ...
        for listing in listings:
            ...
            if not key:
                continue
            seen.add(key)
            row = session.query(LocalsListing).filter(LocalsListing.listing_key == key).first()
            if row is None:
                row = LocalsListing(listing_key=key, first_seen_at=now)
                session.add(row)
            row.title = listing.get("title") or "locals® Wohnung"
            ...
            count += 1
        if seen:
            session.query(LocalsListing).filter(~LocalsListing.listing_key.in_(seen)).update(
                {"is_active": False}, synchronize_session=False
            )
        else:
            session.query(LocalsListing).filter(LocalsListing.is_active.is_(True)).update(
                {"is_active": False}, synchronize_session=False
            )
        session.commit()
        return count
...
def list_active_listings() -> List[Dict]:
    session = DBSession()
    try:
        rows = session.query(LocalsListing).filter(LocalsListing.is_active.is_(True)).all()
...
def list_active_listings_since(cutoff: datetime) -> List[Dict]:
    ...
    session = DBSession()
    try:
        rows = session.query(LocalsListing).filter(
            LocalsListing.is_active.is_(True), LocalsListing.first_seen_at >= cutoff
        ).all()
        return [listing_to_dict(row) for row in rows]
    finally:
        session.close()
...
def delivered_pairs() -> Set[Tuple[int, str]]:
    session = DBSession()
    try:
        return {(int(row.filter_id), str(row.listing_key)) for row in session.query(LocalsDelivery).all()}
    finally:
        session.close()
...
def mark_delivered(filter_id: int, listing_key: str) -> None:
    session = DBSession()
    try:
        exists = session.query(LocalsDelivery).filter(
            LocalsDelivery.filter_id == int(filter_id),
            LocalsDelivery.listing_key == str(listing_key),
        ).first()
        if not exists:
            session.add(LocalsDelivery(filter_id=int(filter_id), listing_key=str(listing_key), sent_at=utc_now()))
            session.commit()
    finally:
        session.close()
    ...
    for filt in filters:
        ...
        for listing in listings:
            ...
            if not key or (filter_id, key) in delivered:
                continue
            if locals_matching.matches_filter(listing, filt):
...
def record_status(status: str, listings_count: int = 0, error: str = "") -> None:
    session = DBSession()
    try:
        row = session.query(LocalsStatus).filter(LocalsStatus.key == STATUS_KEY).first()
        if row is None:
            row = LocalsStatus(key=STATUS_KEY)
            session.add(row)
