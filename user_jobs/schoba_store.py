"""Persistent operations for SCHOBA filters, listings, deliveries, and status.
...
just rooms/area/Nettokaltmiete bounds.
"""
...
from user_jobs import schoba_matching, schoba_parser
...
def parse_optional_number(text) -> Optional[float]:
    return schoba_parser.parse_decimal(text)


def filter_to_dict(row: SchobaFilter) -> Dict:
    return {
...
def listing_to_dict(row: SchobaListing) -> Dict:
    return {
...
def create_filter(
    ...
    try:
        ...
        now = utc_now()
        for listing_row in session.query(SchobaListing).filter(
            SchobaListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
            if schoba_matching.matches_filter(listing, filter_data):
                session.add(SchobaDelivery(
                    filter_id=filter_id,
...
def update_filter(
    ...
    session = DBSession()
    try:
        row = session.query(SchobaFilter).filter(
            SchobaFilter.filter_id == int(filter_id),
            SchobaFilter.user_id == int(user_id),
        ).first()
        if not row:
            return False
        ...
        filter_data = filter_to_dict(row)
        now = utc_now()
        session.query(SchobaDelivery).filter(SchobaDelivery.filter_id == int(filter_id)).delete()
        for listing_row in session.query(SchobaListing).filter(
            SchobaListing.is_active.is_(True)
        ).all():
            listing = listing_to_dict(listing_row)
...
def list_filters(user_id: Optional[int] = None, active_only: bool = False) -> List[Dict]:
    session = DBSession()
    try:
        query = session.query(SchobaFilter)
        if user_id is not None:
            query = query.filter(SchobaFilter.user_id == int(user_id))
        if active_only:
            query = query.filter(SchobaFilter.active.is_(True))
        return [filter_to_dict(row) for row in query.order_by(SchobaFilter.filter_id.asc()).all()]
    finally:
        session.close()
...
def set_filter_active(filter_id: int, active: bool, user_id: Optional[int] = None) -> bool:
    session = DBSession()
    try:
        query = session.query(SchobaFilter).filter(SchobaFilter.filter_id == int(filter_id))
        if user_id is not None:
            query = query.filter(SchobaFilter.user_id == int(user_id))
        row = query.first()
        if not row:
...
def delete_filter(filter_id: int, user_id: Optional[int] = None) -> bool:
    ...
    try:
        ...
        if not row:
            return False
        session.query(SchobaDelivery).filter(
            SchobaDelivery.filter_id == int(filter_id)
    ...
    try:
        ...
        for listing in listings:
            ...
            if not key:
                continue
            seen.add(key)
            row = session.query(SchobaListing).filter(SchobaListing.listing_key == key).first()
            if row is None:
                row = SchobaListing(listing_key=key, first_seen_at=now)
                session.add(row)
            row.title = listing.get("title") or "SCHOBA Wohnung"
            ...
            count += 1
        if seen:
            session.query(SchobaListing).filter(~SchobaListing.listing_key.in_(seen)).update(
                {"is_active": False}, synchronize_session=False
            )
        else:
            session.query(SchobaListing).filter(SchobaListing.is_active.is_(True)).update(
                {"is_active": False}, synchronize_session=False
            )
        session.commit()
        return count
...
def list_active_listings() -> List[Dict]:
    session = DBSession()
    try:
        rows = session.query(SchobaListing).filter(SchobaListing.is_active.is_(True)).all()
...
def list_active_listings_since(cutoff: datetime) -> List[Dict]:
    ...
    session = DBSession()
    try:
        rows = session.query(SchobaListing).filter(
            SchobaListing.is_active.is_(True), SchobaListing.first_seen_at >= cutoff
        ).all()
        return [listing_to_dict(row) for row in rows]
    finally:
        session.close()
...
def delivered_pairs() -> Set[Tuple[int, str]]:
    session = DBSession()
    try:
        return {(int(row.filter_id), str(row.listing_key)) for row in session.query(SchobaDelivery).all()}
    finally:
        session.close()
...
def mark_delivered(filter_id: int, listing_key: str) -> None:
    session = DBSession()
    try:
        exists = session.query(SchobaDelivery).filter(
            SchobaDelivery.filter_id == int(filter_id),
            SchobaDelivery.listing_key == str(listing_key),
        ).first()
        if not exists:
            session.add(SchobaDelivery(filter_id=int(filter_id), listing_key=str(listing_key), sent_at=utc_now()))
            session.commit()
    finally:
        session.close()
    ...
    for filt in filters:
        ...
        for listing in listings:
            ...
            if not key or (filter_id, key) in delivered:
                continue
            if schoba_matching.matches_filter(listing, filt):
...
def record_status(status: str, listings_count: int = 0, error: str = "") -> None:
    session = DBSession()
    try:
        row = session.query(SchobaStatus).filter(SchobaStatus.key == STATUS_KEY).first()
        if row is None:
            row = SchobaStatus(key=STATUS_KEY)
            session.add(row)
